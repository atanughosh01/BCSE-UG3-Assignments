...
#define PER_WRITER_MINIMUM 100
#define PER_WRITER_RANGE 101
...
writer:
{
	...
	while (cnt--)
	{
		...
		l++;
		char buff[50];
		sprintf(buff, "This is line number %5d written by the writer", l);
		sem_wait(read);
		sem_wait(wrt);
	...
	printf("Writer wrote %d lines finished at %ld\n", count,clock());
	return 0;
}
reader:
{
